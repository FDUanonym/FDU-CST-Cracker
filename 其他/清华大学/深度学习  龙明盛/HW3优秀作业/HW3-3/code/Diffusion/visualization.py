...
parser.add_argument('--save_folder', default='images_class')
parser.add_argument('--npz_filename', default='condTrue_16x64x64x3_step100.npz')
...
def main():
    ...
    data = np.load(path)
    step = args.npz_filename.split('_')[-1].split('.')[0]
    print(data.files)
    ...
    for i, img in enumerate(data['arr_0']):
        plt.imshow(img)
        plt.savefig(f"{args.save_folder}/image_{step}_sample0_{i}.png")
