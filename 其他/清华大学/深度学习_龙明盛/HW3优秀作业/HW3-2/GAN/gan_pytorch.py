...
def discriminator_loss(logits_real, logits_fake):
    ...
    #                                                                            #
    ##############################################################################
    target = torch.ones_like(logits_real).squeeze()
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    #                                                                            #
    ##############################################################################
    target = torch.ones_like(scores_real).squeeze()
    loss = F.mse_loss(scores_real.squeeze(), target)
    loss += F.mse_loss(scores_fake.squeeze(), 1 - target)
    loss *= 1 / 2
    return loss
...
def ls_generator_loss(scores_fake):
    ...
    target = torch.ones_like(scores_fake).squeeze()
    loss = 1 / 2 * F.mse_loss(scores_fake.squeeze(), target)
...
def build_dc_classifier(batch_size):
    ...
    flatten = Flatten()
    fc1 = nn.Linear(64 * 2 * 2, 128)
    fc2 = nn.Linear(128, 1)
    return nn.Sequential(
        conv1, act, pooling, conv2, act, pooling, flatten, fc1, act, fc2
    )
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    fc1 = nn.Linear(noise_dim, 512)
    norm1 = nn.BatchNorm1d(512)
    fc2 = nn.Linear(512, 64 * 7 * 7)
    norm2 = nn.BatchNorm1d(64 * 7 * 7)
    unflatten = Unflatten(C=64, H=7, W=7)
