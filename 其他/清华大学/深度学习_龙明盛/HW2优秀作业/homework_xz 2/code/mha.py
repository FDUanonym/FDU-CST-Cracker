...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attn_mask: torch.Tensor=None):
        ...
        ############################################################################
        # NOTE: The provided S and T seem to be wrong.

        # Q, K, V
        # (N, S, H, E/H)
        Q: torch.Tensor = self.query(query).reshape(query.size(0), query.size(1), self.n_head, query.size(2) // self.n_head)
        # (N, T, H, E/H)
        K: torch.Tensor = self.key(key).reshape(key.size(0), key.size(1), self.n_head, key.size(2) // self.n_head)
        # (N, T, H, E/H)
        V: torch.Tensor = self.value(value).reshape(value.size(0), value.size(1), self.n_head, value.size(2) // self.n_head)

        # Attention Score (N, H, S, T)
        attn_score = torch.matmul(Q.transpose(1, 2), K.permute(0, 2, 3, 1))
        nn.TransformerEncoderLayer
        # Apply attn_mask
        if attn_mask is not None:
          attn_mask = attn_mask.reshape(1, 1, attn_mask.size(0), -1).expand_as(attn_score)
          attn_score = torch.masked_fill(attn_score, ~attn_mask, -np.inf)

        attn_score = torch.softmax(attn_score / np.sqrt(self.head_dim), dim=-1)

        # Get result (N, H, S, E/H)
        y = torch.matmul(self.attn_drop(attn_score), V.transpose(1, 2))
        output = self.proj(y.transpose(1, 2).reshape(y.size(0), y.size(2), -1))
