...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.reshape(query.shape[0], query.shape[1], self.n_head, -1)
        key = key.reshape(key.shape[0], key.shape[1], self.n_head, -1)
        value = value.reshape(value.shape[0], value.shape[1], self.n_head, -1)
        query, key = query.transpose(1, 2), key.transpose(1, 2)
        inner_product = torch.matmul(query, key.transpose(2, 3))/np.sqrt(self.head_dim)
        if not attn_mask is None:
            inner_product = inner_product.masked_fill(torch.logical_not(attn_mask), -torch.inf)
        attention = self.attn_drop(nn.functional.softmax(inner_product, dim=3))
        output = torch.matmul(attention, value.transpose(1, 2)).permute(0, 2, 1, 3)
        output = self.proj(output.reshape(output.shape[0], output.shape[1], -1))
        ############################################################################
        return output
