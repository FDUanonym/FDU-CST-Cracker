...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key = key.reshape((N, T, H, E // H)).permute(0, 2, 1, 3)
        query = query.reshape((N, S, H, E // H)).permute(0, 2, 1, 3)
        value = value.reshape((N, T, H, E // H)).permute(0, 2, 1, 3)

        mask = torch.zeros((S, T))
        if attn_mask is not None:
            mask.data.masked_fill_(~attn_mask, -torch.inf)

        output = torch.softmax(torch.matmul(query, key.transpose(-2, -1)) / np.sqrt(E // H) + mask, dim = -1)
